const NUMBER_OF_MILLISECONNDS_IN_ONE_MINUTE = 60000;
...
export const convertTimeToString = (time: number): string => {
  ...
  const seconds = Math.floor((time - minutes * NUMBER_OF_MILLISECONNDS_IN_ONE_MINUTE) / NUMBER_OF_MILLISECONDS_IN_ONE_SECOND);
